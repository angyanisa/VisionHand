...
async def main():
    ...
    gForce.connect(device_address)
    print(f"Connected directly to {device_address}")
    gForce.setEmgRawDataConfig(
        sampRate,
        channelMask,
        dataLen,
        resolution,
    ...
    gForce.setDataNotifSwitch(DataNotifFlags["DNF_EMG_RAW"], set_cmd_cb, 1000)
    await asyncio.sleep(1)
    gForce.startDataNotification(ondata)
    print("Press enter to stop...")
    await asyncio.to_thread(lambda: input())
    print("Stopping...")
    gForce.stopDataNotification()
    await asyncio.sleep(1)
    gForce.setDataNotifSwitch(DataNotifFlags["DNF_OFF"], set_cmd_cb, 1000)
    gForce.disconnect()
