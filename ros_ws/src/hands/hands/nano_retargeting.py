...
Joint layout from nanohand_test.urdf (PyBullet index order):
  ...
  2:  pinky_curl_pip     (mimic of pinky_curl,  multiplier=0.7)
  3:  pinky_curl_dip     (mimic of pinky_curl,  multiplier=0.5)
  4:  ring_wiggle        (independent)
  5:  ring_curl          (independent — MCP)
  6:  ring_curl_pip      (mimic of ring_curl,   multiplier=0.7)
  7:  ring_curl_dip      (mimic of ring_curl,   multiplier=0.5)
  8:  middle_wiggle      (independent)
  9:  middle_curl        (independent — MCP)
  10: middle_curl_pip    (mimic of middle_curl, multiplier=0.7)
  11: middle_curl_dip    (mimic of middle_curl, multiplier=0.5)
  ...
  18: thumb_curl_pip     (mimic of thumb_curl,  multiplier=0.7)
  19: thumb_curl_dip     (mimic of thumb_curl,  multiplier=0.5)
...
class NanoRetargeting(Node):
    def __init__(self):
        ...
        self.joints = {
            ...
            'thumb_wiggle': {'pb_idx': 16, 'limits': [-0.5,   0.5  ]},
        ...
        self.mimic_joints = {
            # Pinky
            2:  ('pinky_curl',  1,  0.7, 0.0),   # pinky_curl_pip  (PIP)
            3:  ('pinky_curl',  1,  0.5, 0.0),   # pinky_curl_dip  (DIP)
            # Ring
            6:  ('ring_curl',   5,  0.7, 0.0),   # ring_curl_pip
            7:  ('ring_curl',   5,  0.5, 0.0),   # ring_curl_dip
            # Middle
            10: ('middle_curl', 9,  0.7, 0.0),   # middle_curl_pip
            11: ('middle_curl', 9,  0.5, 0.0),   # middle_curl_dip
            # Index
            14: ('index_curl',  13, 0.7, 0.0),   # index_curl_pip
            15: ('index_curl',  13, 0.5, 0.0),   # index_curl_dip
            # Thumb
            18: ('thumb_curl',  17, 0.7, 0.0),   # thumb_curl_pip
            19: ('thumb_curl',  17, 0.5, 0.0),   # thumb_curl_dip
        }
        ...
        self.joint_mapping = {
            'thumb_curl':    'LeftDigit1Carpometacarpal_flexion',
            'thumb_wiggle':  'LeftDigit1Carpometacarpal_ulnarDeviation',
            'index_curl':    'LeftDigit2Metacarpophalangeal_flexion',
            'index_wiggle':  'LeftDigit2Metacarpophalangeal_ulnarDeviation',
            'middle_curl':   'LeftDigit3Metacarpophalangeal_flexion',
            'middle_wiggle': 'LeftDigit3Metacarpophalangeal_ulnarDeviation',
            'ring_curl':     'LeftDigit4Metacarpophalangeal_flexion',
            'ring_wiggle':   'LeftDigit4Metacarpophalangeal_ulnarDeviation',
            'pinky_curl':    'LeftDigit5Metacarpophalangeal_flexion',
            'pinky_wiggle':  'LeftDigit5Metacarpophalangeal_ulnarDeviation',
        }
